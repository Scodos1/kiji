...
function Brand({ className = '' }) {
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-600 font-display text-sm font-bold text-white shadow-sm shadow-brand-600/20">
        K
      </div>
...
export default function Layout() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50/80">
      <aside className="fixed inset-y-0 left-0 hidden w-60 flex-col border-r border-slate-200/60 bg-white px-3 py-5 lg:flex">
        ...
        <nav className="mt-7 flex flex-1 flex-col gap-0.5">
          {NAV.map((item) => (
            ...
            >
              ...
              {item.highlight && (
                <span className="rounded-full bg-brand-600 px-2 py-0.5 text-[10px] font-bold text-white shadow-sm">
                  AI
                </span>
              )}
            </NavLink>
        ...
        <div className="border-t border-slate-100 pt-3">
          ...
          <button
            ...
            className="mt-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600"
          >
            <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex min-h-screen flex-1 flex-col lg:pl-60">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200/60 bg-white/80 px-4 py-3 backdrop-blur-sm lg:px-8">
          <div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white shadow-sm shadow-brand-600/20">
              {firstName[0]?.toUpperCase()}
            </div>
          </div>
        </header>

        <div className="flex items-center justify-between border-b border-slate-200/60 bg-white px-4 py-3 lg:hidden">
          <Brand />
          <button
        ...
        {mobileOpen && (
          <nav className="border-b border-slate-200/60 bg-white px-4 py-2 lg:hidden">
            {NAV.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
        ...
        <nav className="sticky bottom-0 z-30 grid grid-cols-5 border-t border-slate-200/60 bg-white/90 px-2 py-2 backdrop-blur-sm lg:hidden">
          {MOBILE_NAV.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
